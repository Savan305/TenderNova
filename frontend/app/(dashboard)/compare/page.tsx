    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(280px,360px)_1fr]">
        ...
        <div className="space-y-5">
          ...
          <div className="glass rounded-lg p-5"><h2 className="font-semibold">Metrics radar</h2><div className="mt-4 h-72 sm:h-80">{mounted ? <ResponsiveContainer><RadarChart data={chartData}><PolarGrid /><PolarAngleAxis dataKey="tender" tick={{ fontSize: 11 }} /><Tooltip />{['eligibility', 'budget', 'timeline', 'risk'].map((key, index) => <Radar key={key} dataKey={key} stroke={['#06B6D4', '#6366F1', '#22C55E', '#F59E0B'][index]} fill={['#06B6D4', '#6366F1', '#22C55E', '#F59E0B'][index]} fillOpacity={0.08} />)}</RadarChart></ResponsiveContainer> : null}</div></div>
